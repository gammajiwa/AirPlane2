...
public class EnemyScriptableObject : BaseScriptableObject    // Change ID enum, and Data class reference in the comment below
{
	[BoxGroup("General")]
	[HorizontalGroup("General/Split", 0.8f, LabelWidth = 135)]
	[VerticalGroup("General/Split/Left")] [ReadOnly] public EnemyID ID;
	...
	private string filePath = EnemyData.GetFilePath();	// Change this to corresponding Data class

	public EnemyScriptableObject() { }

	public EnemyScriptableObject(EnemyID identifier) {
		ID = identifier;
		Load();
		Save();
	}
